...
public class DebugDataCollector {
    ...
    public static void handleDebugValue(Object valueObj, Map<String, String> variables) {
        ...
        try {
            Class<?> xValueClass = xValue.getClass();

            // IntelliJ IDEA: handle Java debug values
            if (xValueClass.getName().contains("com.intellij.debugger.engine")) {
                Method getNameMethod = xValueClass.getMethod("getName");
                Method calcValueNameMethod = xValueClass.getMethod("calcValueName");

                String name = (String) getNameMethod.invoke(xValue);
                String value = (String) calcValueNameMethod.invoke(xValue);

                // Handle exception values explicitly in IntelliJ
            ...
            // PyCharm: handle Python debug values (__exception__ and normal variables)
            else if (xValueClass.getName().contains("PyDebugValue")) {
                Method getNameMethod = xValueClass.getMethod("getName");
                Method getValueMethod = xValueClass.getMethod("getValue");

                String name = (String) getNameMethod.invoke(xValue);
                String value = (String) getValueMethod.invoke(xValue);

                if ("__exception__".equals(name)) {
                    // Special-case handling to mirror IntelliJ exceptions
                    variables.put("EXCEPTION", value);
            ...
            // Fallback for unknown object types
                ...
                variables.put("unknown", xValue.toString());
            }
