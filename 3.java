...
/**
 * Kotlin debug collector (very similar to Java, but can be extended for Kotlin-specific PSI etc)
 */
public class DebugDataCollectorKotlin implements DebugDataCollectorInterface {
    private static final Logger logger = Logger.getInstance(DebugDataCollectorKotlin.class);
    private static final DebugDataCollectorKotlin instance = new DebugDataCollectorKotlin();
    ...
    private DebugDataCollectorKotlin() {}

    public static DebugDataCollectorKotlin getInstance() { return instance; }
            ...
            public void addChildren(@NotNull XValueChildrenList children, boolean last) {
                for (int i = 0; i < children.size(); i++) {
                    ...
                    if (name != null && name.toLowerCase().contains("exception")) {
                        ExceptionDetail detail = new ExceptionDetail("Kotlin Exception", "Exception", "No stacktrace extracted", "unknown", -1);
                        instance.latestException = detail;
                        callback.accept(new ContextItem(detail, true, ContextItem.Type.EXCEPTION));
                        return;
                    }
                }
                // Fallback: just collect snapshot if no exception info found
