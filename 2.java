...
/**
 * Python/PyCharm debug collector
 */
public class DebugDataCollectorPython implements DebugDataCollectorInterface {
    private static final Logger logger = Logger.getInstance(DebugDataCollectorPython.class);
    private static final DebugDataCollectorPython instance = new DebugDataCollectorPython();
    ...
    private DebugDataCollectorPython() {}

    public static DebugDataCollectorPython getInstance() { return instance; }
                ...
                public void addStackFrames(@NotNull List<? extends XStackFrame> frames, boolean last) {
                    for (XStackFrame frame : frames) {
                        ...
                        if (pos != null) {
                            ...
                            stackItems.add(new StackItem(file, line, "", language));
                        }
                    }
            ...
            public void addChildren(@NotNull XValueChildrenList children, boolean last) {
                for (int i = 0; i < children.size(); i++) {
                    ...
                    if ("__exception__".equals(name)) {
                        // Parse python exception tuple: (type, value, traceback)
                        // For brevity, we just set message as type+value, real implementation should traverse children
                        ExceptionDetail detail = new ExceptionDetail("Python Exception", "Exception", "Traceback not extracted", "unknown", -1);
                        instance.latestException = detail;
                        callback.accept(new ContextItem(detail, true, ContextItem.Type.EXCEPTION));
                        return;
                    }
                }
                // Fallback: just collect snapshot if no Python exception info found
