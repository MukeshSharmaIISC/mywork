 ...
 * DebugDataCollector
 *
 * Improved PyCharm support:
 *  - safe PSI reads via runReadAction
 *  - per-descriptor attempt caps to avoid re-processing/loops
 *  - single-shot evaluator fallback for Python repr()
 *  - cycle detection, node / children caps to avoid OOMs
 *
 * Paste into your project and rebuild plugin for PyCharm.
 */
public class DebugDataCollector {
    ...
    private static Boolean cachedIsPyCharm = null; // cached detection

    // Per-snapshot / per-thread data to prevent rework
    private static final ThreadLocal<Map<Object, Integer>> descriptorAttemptCount = ThreadLocal.withInitial(HashMap::new);
    private static final ThreadLocal<Set<String>> evaluatorTriedFor = ThreadLocal.withInitial(HashSet::new);
    private static final int MAX_DESCRIPTOR_ATTEMPTS = 3;
    ...
    public void clearDebugData() { latestSnapshot.clear(); latestStack.clear(); latestException = null; }
    ...
    public static void collectStackItems(XDebugProcess debugProcess, Consumer<ContextItem> callback) {
        ...
        if (stack == null) { callback.accept(new ContextItem(stackItems, false, ContextItem.Type.STACK)); return; }
    ...
    public static void collectSnapshot(XStackFrame currentStackFrame, Consumer<ContextItem> callback) {
        ...
        // guards for snapshot (cycle detection, node caps)
        final AtomicInteger totalNodes = new AtomicInteger(0);
        final Set<Object> visited = Collections.newSetFromMap(new IdentityHashMap<>());

        // per-snapshot evaluator cache
        evaluatorTriedFor.get().clear();
        descriptorAttemptCount.get().clear();
        ...
        try {
            currentStackFrame.computeChildren(new XCompositeNode() {
                ...
                public void addChildren(@NotNull XValueChildrenList children, boolean last) {
                    ...
                    if (children.size() == 0) { complete(); return; }
                ...
                private void complete() {
                    ...
                    // clear per-snapshot ThreadLocal caches
                    descriptorAttemptCount.remove();
                    evaluatorTriedFor.remove();
    ...
    /* -------------------- Generic XValue traversal (Java/native) -------------------- */
    ...
    private static void collectPyCharmValueAndChildren(XStackFrame frame, XValue value, MutableSnapshotItem parent,
        ...
        try {
            ...
            // descriptor attempt guard: avoid re-reflecting same descriptor many times
            if (pyDebugValue != null) {
                Map<Object, Integer> attempts = descriptorAttemptCount.get();
                Integer c = attempts.get(pyDebugValue);
                if (c == null) c = 0;
                if (c >= MAX_DESCRIPTOR_ATTEMPTS) {
                    parent.value = "[unavailable: descriptor attempts exhausted]";
                    finishOnce.run();
                    return;
                }
                attempts.put(pyDebugValue, c + 1);
            }
            ...
            if (pyDebugValue != null) {
                try {
                    ...
                    // fall through to computePresentation fallback
                }
            }

            // Fallback: presentation + computeChildren (same pattern as generic collector)
            xValueLocal.computePresentation(new XValueNode() {
                @Override
                public void setPresentation(@Nullable Icon icon, @NotNull XValuePresentation presentation, boolean hasChildren) {
                    try {
                    ...
                    // If value still unavailable, try single-shot evaluator repr(name) if possible
                    try {
                        if ((parent.value == null || parent.value.isEmpty() || "unavailable".equals(parent.value))
                                && parent.name != null && frame.getEvaluator() != null) {
                            Set<String> tried = evaluatorTriedFor.get();
                            if (!tried.contains(parent.name)) {
                                tried.add(parent.name);
                                String safeExpr = "repr(" + parent.name + ")";
                                try {
                                    frame.getEvaluator().evaluate(safeExpr, new XDebuggerEvaluator.XEvaluationCallback() {
                                        @Override
                                            ...
                                            try {
                                                result.computePresentation(new XValueNode() {
                                                    ...
                                                    public void setPresentation(@Nullable Icon icon, @NotNull XValuePresentation presentation, boolean hasChildren) {
                                                        ...
                                                        if (evalValue != null && !evalValue.isEmpty()) parent.value = evalValue;
                                                    }
                                                    @Override public void setFullValueEvaluator(@NotNull XFullValueEvaluator fullValueEvaluator) {}
                                    ...
                                    logger.debug("evaluator invocation failed: " + e.getMessage());
                                }
                            }
                        }
                    } catch (Throwable ee) {
                        logger.debug("evaluator fallback threw: " + ee.getMessage());
                    }
    ...
    private static List<Object> tryGetPyChildren(Object pyDebugValue) {
        try {
            ...
            // inspect fields/methods heuristically
    ...
    private static List<Object> inspectDescriptorForChildren(Object descriptor) {
        ...
        try {
            ...
            // fallback: try toArray / asList style zero-arg methods
            for (Method m : methods) {
                try {
                    if (m.getParameterCount() != 0) continue;
                    String name = m.getName().toLowerCase();
        ...
        try {
            ...
            ApplicationManager.getApplication().runReadAction(() -> {
                try {
                    ...
                    if (functionElement == null) return;
                    ...
                    resultRef.set(clipped.toString().trim());
                } catch (Throwable e) {
                    logger.debug("extractEnclosingFunctionReflective(read) failed: " + e.getMessage());
                }
            });
